...
const isExternalDefault = (regex, chunkName, mod) =>
  ...
  (mod.issuerName === null ||
    mod.identifier.indexOf(path.dirname(mod.issuerName).replace(/^\./, '')) ===
      -1)
...
export const getRelRegex = chunkName => {
  if (!reCache[chunkName])
    reCache[chunkName] = new RegExp(
      `/${chunkName.replace(relativePkg, '')}((/index)*.(m?jsx?|tsx?))+`
    )
...
export default function getChunkScripts(
  stats,
  cache,
  {async = true, defer, preload}
) {
  let scripts = [],
    preloads = [],
  ...
  if (typeof preload === 'object') {
    preloadAttrs = Object.keys(preload)
      .map(k => `${k}="${preload[k]}"`)
      .join(' ')
  ...
  let i,
    ...
    k,
    moduleIds = {},
    chunkNames = cache.getChunkNames()
  ...
  for (let chunk of findChunks(stats, chunkNames)) {
    for (i = 0; i < chunk.files.length; i++) {
      ...
      if (preload) {
        ...
        if (chunk.entry || chunk.initial) preloads.unshift(p)
  ...
  scripts.unshift(
    `<script id="__INITIAL_BROKER_CHUNKS__" type="application/json">${JSON.stringify(
      moduleIds
    )}</script>`
  )
